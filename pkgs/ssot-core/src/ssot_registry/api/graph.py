...
<body>
  ...
  <script>
    ...
    const els = Object.fromEntries(["search","mode","edgeType","engine","ribbon","forceCutoff","engineReadout","depth","limit","xScale","yScale","families","results","nodeCount","edgeCount","fit","focus","deselect","visibleStats","selected","summary","scaleReadout","showAll","hideAll"].map(id => [id, document.getElementById(id)]));
    ...
    for (const el of [els.search,els.mode,els.edgeType,els.engine,els.ribbon,els.forceCutoff,els.depth,els.limit,els.xScale,els.yScale]) el.addEventListener("input",()=>{ if(el!==els.depth && el!==els.engine && el!==els.ribbon && el!==els.forceCutoff) centerId=null; update(); });
