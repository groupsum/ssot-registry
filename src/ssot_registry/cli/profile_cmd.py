...
def register_profile(subparsers: argparse._SubParsersAction) -> None:
    ...
    add_optional_bool_argument(
        ...
        default=True,
        help_text="Allow feature plan.target_claim_tier to override profile claim tier.",
    )
    ...
    link = profile_sub.add_parser("link", help="Link profile to features or profiles.")
    ...
    unlink = profile_sub.add_parser("unlink", help="Unlink profile from features or profiles.")
    ...
    evaluate = profile_sub.add_parser("evaluate", help="Evaluate profile passing status.")
    add_path_argument(evaluate)
    evaluate.add_argument("--profile-id", required=True)
    evaluate.set_defaults(func=run_evaluate)

    verify = profile_sub.add_parser("verify", help="Alias for evaluate.")
...
def run_create(args: argparse.Namespace) -> dict[str, object]:
    ...
    return create_entity(args.path, "profiles", row)
...
def run_evaluate(args: argparse.Namespace) -> dict[str, object]:
    registry_path, _repo_root, registry = load_registry(args.path)
    index = build_index(registry, [])
    profile = index["profiles"].get(args.profile_id)
    if profile is None:
