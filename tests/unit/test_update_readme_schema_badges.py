...
class UpdateReadmeSchemaBadgesTests(unittest.TestCase):
    def test_update_readme_rewrites_managed_badges_and_version_reference(self) -> None:
        with workspace_tempdir() as temp_dir:
            ...
            expected_coverage = update_readme_schema_badges.migration_coverage_label().replace("/", "%2F")
            self.assertIn(f"migration%20coverage-{expected_coverage}-brightgreen", updated)
