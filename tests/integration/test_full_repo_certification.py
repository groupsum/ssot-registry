...
from ssot_registry.api import certify_release, load_registry, validate_registry
from ssot_registry.util.errors import GuardError
from tests.helpers import workspace_tempdir


class FullRepoCertificationTests(unittest.TestCase):
    def test_full_repo_release_fails_closed_from_temp_copy(self) -> None:
        with workspace_tempdir() as temp_dir:
            ...
            shutil.copytree(PROJECT_ROOT / "pkgs" / "ssot-pack-contracts", repo / "pkgs" / "ssot-pack-contracts")

            validation = validate_registry(repo)
            self.assertTrue(validation["passed"], validation)

            with self.assertRaisesRegex(GuardError, "requires all linked evidence rows to be passed"):
                certify_release(repo, release_id="rel:full-cert", write_report=True)
